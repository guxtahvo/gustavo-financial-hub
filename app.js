...
function getAllocationCandidatesForMigration(key=monthKey()){const budget=getBudgetWithoutAllocations(key),configured=new Set((state.monthlyAllocations||[]).map(a=>a.category));return budgetCategoryDefs().filter(def=>Number(budget[def.name]||0)>0&&!configured.has(def.name)).map(def=>({name:def.name,category:def.name,amount:Number(budget[def.name])||0,type:def.type}));}
...
function migrateBudgetToAllocations(){const candidates=getAllocationCandidatesForMigration();if(!candidates.length){showToast('Não há valores do orçamento prontos para converter.');return;}if(!confirm(`Encontramos ${candidates.length} valor(es) do orçamento que podem virar regras automáticas.\n\n${candidates.map(c=>`${c.name} ${currency(c.amount)}`).join('\\n')}\n\nConfigurar agora?`))return;for(const c of candidates)state.monthlyAllocations.push(normalizeMonthlyAllocation(c));saveState();renderAll();showToast('Destinos configurados a partir do orçamento.');}
function bindAllocationEvents(){const add=document.getElementById('addAllocationBtn');if(add)add.addEventListener('click',()=>openAllocationModal());const form=document.getElementById('allocationForm');if(form)form.addEventListener('submit',saveMonthlyAllocation);document.getElementById('allocationType')?.addEventListener('change',e=>populateAllocationCategories(e.target.value,document.getElementById('allocationCategory')?.value||''));document.getElementById('allocationUseOverride')?.addEventListener('change',e=>{const input=document.getElementById('allocationOverrideAmount');if(input)input.disabled=!e.target.checked;});document.getElementById('allocationMonthSelect')?.addEventListener('change',()=>{allocationSelectedMonth=document.getElementById('allocationMonthSelect').value;renderAllocationSummary(allocationSelectedMonth);});document.getElementById('allocationPrevMonth')?.addEventListener('click',()=>{allocationSelectedMonth=addMonthsKey(allocationSelectedMonth,-1);renderAllocationMonthSelect();renderAllocationSummary(allocationSelectedMonth);});document.getElementById('allocationNextMonth')?.addEventListener('click',()=>{allocationSelectedMonth=addMonthsKey(allocationSelectedMonth,1);renderAllocationMonthSelect();renderAllocationSummary(allocationSelectedMonth);});document.getElementById('allocationList')?.addEventListener('click',e=>{const btn=e.target.closest('[data-allocation-action]');if(btn){const action=btn.dataset.allocationAction,id=btn.dataset.id;if(action==='edit')openAllocationModal(id);else if(action==='toggle')toggleMonthlyAllocation(id);else if(action==='delete')deleteMonthlyAllocation(id);else if(action==='migrate')migrateBudgetToAllocations();else if(action==='edit-summary')openAllocationModal();return;}const amount=e.target.closest('.allocation-inline-edit');if(amount)inlineEditAllocation(amount.dataset.id,document.getElementById('allocationMonthSelect').value);});}
