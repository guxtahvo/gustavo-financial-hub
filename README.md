...
### Patch — Mobile Flags + Configurar a partir do orçamento

Este patch corrige duas inconsistências pontuais sem criar nova arquitetura:

- `country-flag-asset` mantém o mesmo atlas local de 960×624 e agora usa `background-size: 360px 234px` também em mobile, mantendo a escala compatível com `--flag-x/--flag-y` calculada no JavaScript.
- `normalizeMonthlyAllocation()` foi restaurada como a função única de normalização das regras de `monthlyAllocations`; `allocationTypeLabel()` também foi restaurada para eliminar referências ausentes no renderizador.
- `Configurar a partir do orçamento` passou a ficar disponível mesmo quando já existem regras. A migração compara categorias existentes, não duplica regras, pode atualizar valores mediante confirmação e preserva overrides, prioridade, notas e `linkedCommitmentId`.
- A conversão de `Viagens` mantém `type: 'travel'` e `role: 'travel'`, sem criar transações reais.
- O service worker recebeu apenas um novo identificador de cache (`gfh-static-v1-salaryauto1`) para que instalações PWA existentes baixem os arquivos estáticos corrigidos.

`world-map-data.js` e `icons/flags-atlas.png` foram preservados byte-for-byte.
